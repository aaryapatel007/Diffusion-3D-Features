...
import numpy as np
from diffusion_copy import add_texture_to_render
from dino import get_dino_features
from render import batch_render
from render_point_cloud_copy import batch_render as batch_render_point_cloud
...
def get_features_per_vertex(
    ...
    if not ply:
        ...
        batched_renderings, normal_batched_renderings,camera, depth, _ = batch_render_point_cloud(
            device=device,points=mesh.points_list()[0], num_views=num_views,H=H, W=W,
            use_normal_map=use_normal_map,return_images=return_image
            )
